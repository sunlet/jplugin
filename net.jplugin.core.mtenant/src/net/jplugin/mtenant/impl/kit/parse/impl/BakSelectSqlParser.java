...
public class BakSelectSqlParser implements SqlParser {
	...
	public String parse(String sourceSql, Map<String, Object> params,
		...
		for (int i = 0; i < length; i++) {
			if ("from".equals(sqlArray[i])) {
				checkTable(sqlArray, i + 1, ignoreTables, map, tablemap,
						aliasmap);
			}
		}
		...
		for (int i : nmap.keySet()) {
			if (i + 1 <= list.size()) {
				list.add(i, SqlHelper.createWheres(aliasmap, tablemap.get(i),
						params, nmap.get(i)));
			} else {
				list.add(SqlHelper.createWheres(aliasmap, tablemap.get(i),
						params, nmap.get(i)));
			}
		}
	...
	private void checkTable(String[] sqlArray, int pos,
			List<String> ignoreTables, TreeMap<Integer, String> map,
			TreeMap<Integer, List<String>> tablemap,
		...
		for (int i = pos; i < sqlArray.length; i++) {
			...
			if (("left".equals(sqlArray[i]) || "right".equals(sqlArray[i])
					|| "inner".equals(sqlArray[i]) || "full"
						.equals(sqlArray[i])) && "join".equals(sqlArray[i + 1])) {
				flag = false;
			}

			if (")".equals(sqlArray[i]) || "limit".equals(sqlArray[i])
					|| ("order".equals(sqlArray[i]))
					&& "by".equals(sqlArray[i + 1])) {
				position = i;
				break;
			}
			if (flag) {
