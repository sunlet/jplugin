...
public class MultidbSqlHelperTest {
	...
	public void test(){
		...
		source = "select * from (select abcid, bbb as bbb1, ccc AS ccc1 from abc)";
		target = "select * from( select abcid , bbb as bbb1 , ccc AS ccc1 from sss.abc )";
		check(source,target);
		
		source = "SELECT platformSysNo,deptCode,deptName,typeFlag,yn FROM \n"+
			    "(\n"+
			    "SELECT platformSysNo ,provinceId AS deptCode, provinceName deptName,yn,\n"+
				"CASE parentid WHEN '0' THEN '总部' ELSE '分总部'  END  typeFlag FROM province\n"+
			    "UNION\n"+
			    "SELECT platformSysNo, storeId AS deptCode, storeName AS deptName,yn,\n"+
				"CASE type WHEN 1 THEN '门店' ELSE '配送' END  typeFlag FROM store) t";
