...
//		plugin.addExtension(Extension.create(Plugin.EP_MYBATIS_INCEPT, inceptorClass));
	...
	public static void autoBindMapperExtension(AbstractPlugin p, String pkgPath) {
		...
		for (Class c : p.filterContainedClasses(pkgPath, BindMapperSet.class)) {
			Annotation[] annos = c.getAnnotationsByType(BindMapper.class);
			checkSameDs(annos,c);
	...
	private static void checkSameDs(Annotation[] annos, Class c) {
		String ds = null;
		boolean first=true;
		HashSet<String> set = new HashSet();
		for (Annotation a:annos) {
			BindMapper anno = (BindMapper) a;
			if (first) {
				ds = anno.dataSource();
				set.add(ds);
				first=false;
			}else {
				if (set.contains(anno.dataSource())) {
					throw new RuntimeException("Duplicated datasource for mapping class:"+c.getName());
				}else {
					set.add(anno.dataSource());
				}
			}
		}
	}

	private static void handleOne(AbstractPlugin p, Class c, Annotation a) {
		BindMapper anno = (BindMapper) a;
